...
export const CaseStudyModal = ({ modalData }) => {
    const caseStudy = caseStudies.find(
        (obj) => obj.name === modalData.data.name,
    );
    ...
    return (
        <div
            className={`csm-main-container ${isLoading ? 'csm-loading' : ''}`}
        ...
        >
            ...
            */}
            <div
                className="csm-expanded-image-container"
                style={
                    expandedImage
                        ? { visibility: 'visible' }
                        : { visibility: 'hidden' }
            ...
            >
                <div>
                    <img
                        src={`${images.caseStudyImagePath}${currentPage.images[0]}`}
                    />
                </div>
            </div>
            {/*
            ...
            >
                ...
                */}
                <div className="csm-title-container">
                    ...
                    <div
                        className="text-box-content csm-title"
                        style={{ opacity: '.95' }}
                    >
                        {`${currentPage.title}`}
                    </div>
                ...
                */}
                <div
                    className="csm-text-container"
                    style={{ pointerEvents: 'auto' }}
                >
                    <div className="csm-text">{currentPage.text}</div>
                </div>
                {/*
                    Image
                    TODO:  ACCESIBILITY
                ...
                */}
                <div
                    className={`csm-next-page ${pageIndex === caseStudy.pages.length - 1 ? 'csm-fade' : 'csm-animate'}`}
                    onClick={() => changeIndex(1)}
                >
                    <img src={images.navArrow} />
                </div>
                <div
                    className={`csm-previous-page ${pageIndex === 0 ? 'csm-fade' : 'csm-animate'}`}
                    onClick={() => changeIndex(-1)}
                >
                    <img src={images.navArrow} />
                </div>
