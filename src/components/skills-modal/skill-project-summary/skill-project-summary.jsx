...
export const SkillProjectSummary = ({ project, setModalData }) => {
            ...
            .map((skill, index) => {
                return (
                    <div
                        className="sps-skill"
                        style={{
                            backgroundImage: 'url(/Skill-Button-175-50.png)',
                        }}
                        key={`${skill}${index}`}
                        onClick={(e) => {
                            e.stopPropagation();
    ...
    const linkButton = (text, target, index) => {
        return (
            <div
                className="sps-link"
                style={{ backgroundImage: 'url(/Link-Button-379-54.png)' }}
                key={`${index}${text}`}
    ...
    return (
        <div
            ...
            onClick={() =>
                setModalData({ type: 'project', data: { name: project.name } })
            }
        >
            <div className="sps-image-container">
                <img
                    src={`${images.projectImagePath}${project.image}`}
                    className="sps-image"
                />
            </div>
            <div className="sps-project-name">{project.name}</div>
